...
\begin{tabular}{ll}
  \toprule
  Systematic uncertainty & Short description & Reference\\\
  \midrule
  {\bfseries Event}&&\\
...
EL\_EFF\_Trigger\_Total\_1NPCOR\_PLUS\_UNCOR &  trigger efficiency uncertainty\\%& Section 12.2.2. in Ref. \cite{VHobjectsupportnote}\\
EL\_EFF\_Reco\_Total\_1NPCOR\_PLUS\_UNCOR &  reconstruction efficiency uncertainty\\%& Section 12.3.1. in Ref. \cite{VHobjectsupportnote}\\
EL\_EFF\_ID\_Total\_1NPCOR\_PLUS\_UNCOR &  ID efficiency uncertainty\\%& Section 12.3.1. in Ref. \cite{VHobjectsupportnote}\\
EL\_EFF\_Iso\_Total\_1NPCOR\_PLUS\_UNCOR &  isolation efficiency uncertainty\\%& Section 12.3.1. in Ref. \cite{VHobjectsupportnote}\\
EG\_SCALE\_ALL &        energy scale uncertainty         \\%& Section 12.3.2. in Ref. \cite{VHobjectsupportnote}\\         
EG\_RESOLUTION\_ALL &    energy resolution uncertainty    \\%& Section 12.3.2. in Ref. \cite{VHobjectsupportnote}\\
{\bfseries Muons}\\
MUON\_EFF\_TrigStatUncertainty &  \multirow{2}{*}{trigger efficiency uncertainty} \\%& \multirow{2}{*}{Section 12.2.2. in Ref. \cite{VHobjectsupportnote}}\\ % to verify
MUON\_EFF\_TrigSystUncertainty & & \\ % to verify
MUON\_EFF\_RECO\_STAT &  \multirow{2}{*}{reconstruction and ID efficiency uncertainty for muons with $p_T$\ $>$ 15 GeV} \\%& \multirow{2}{*}{Section 12.4.1. in Ref. \cite{VHobjectsupportnote}}\\  
...
JET\_CR\_JET\_EffectiveNP\_Detector1,2 & energy scale uncertainty from the in situ analyses (detector) \\%& Section 12.5.1. in Ref. \cite{VHobjectsupportnote}\\ % note: check diff items in CR scheme
...
JET\_CR\_JET\_BJES\_Response &  \\%& Section 12.5.1. in Ref. \cite{VHobjectsupportnote}\\ % note: details to fill
JET\_CR\_JET\_Flavor\_Composition & energy scale uncertainty on $VV$ and $VH$ sample's flavour composition \\%& Section 12.5.1. in Ref. \cite{VHobjectsupportnote}\\ % note: decorr scheme ?
...
JET\_CR\_JET\_Pileup\_OffsetMu & energy scale uncertainty on pile-up (Mu dependent) \\%& Section 12.5.1. in Ref. \cite{VHobjectsupportnote}\\ % note: diff with NPV ???
...
FT\_EFF\_Eigen\_extrapolation\_from\_charm & $b$-tagging efficiency uncertainty on tau jets \\%& Section 12.7. in  Ref. \cite{VHobjectsupportnote}\\
{\bfseries MET}\\
...
MET\_JetTrk\_Scale    & track MET scale uncertainty due to tracks in jets \\%&Section 12.6. in  Ref. \cite{VHobjectsupportnote}\\
{\bfseries Taus}\\
TAUS\_TRUEHADTAU\_SME\_TES\_DETECTOR & energy scale uncertainty: single-particle response + threshold \\%& \\ % note: my knowledge of tau uncertainties is very limited
