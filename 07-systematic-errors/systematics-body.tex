...
described in table~\ref{tab:bc-filters}.

% For Sherpa 2.2 the same filter was used for W and Z
%The charm filter for the $W$+jets samples is tighter than the equivalent for the $Z$+jets samples because of the large 
%contribution to the total $W$+jets cross section of $W$+$c$ events. Note that the $|\eta|$ range is looser for the CFilter $W$ samples because an additional jet requirement is applied at generation level in order to reduce the larger $W$+$c$ jet cross section.




All $V$+jets samples with the corresponding max($H_T$,$p_T^V$) slices and
heavy-flavour filters are listed in Tables~\ref{tabular:mc_samples_Wjets}
\ref{tabular:mc_samples_Zlljets}, and \ref{tabular:mc_samples_Zvvjets}.
