...
channel but as they don't exist in the 0-- and 1--lepton channel Monte-Carlo
generator predictions and theoretical uncertainties are used instead. A summary
of the nuisance parameters the control the $t\bar{t}$ systematic uncertainties
can be found in table~\ref{tab:ttbar-systs}.
\input{07-systematic-errors/ttbar-table.tex}

% The modelling of top-quark pair production within the fiducial phase space of
% the analysis differs between the 0-/1-lepton and 2-lepton channel due to the
% differing definitions of the $V$-boson system. The $V$-boson system in the
% 0-/1-lepton channel typically represents a single $W$-boson from a single
% top-quark decay, meanwhile in the 2-lepton channel the $V$-boson candidate is
% constructed from the $ll$-system, which corresponds to two leptons from two
% different top-quarks. This difference is reflected in the different flavour
% compositions of the $t\bar{t}$ process for the 0+1-lepton and 2-lepton channels,
% as the possibility of a hadronically decaying $W$-bosons in the 0-/1-lepton
% channel yields a larger non-$bb$ flavour sub-component.


% Consequently, two different methodologies are used for the 0-/1-lepton and
% 2-lepton: Monte Carlo based prediction and associated theory uncertainties are
% used in the 0-/1-lepton, meanwhile the 2-lepton categorises the $ll$-system into
% one of two categorises, either same-flavour ($ee/\mu\mu$) or opposite-flavour
% ($e\mu$), where the former is an enriched $Z$+Jets region, whilst the latter
% yields a pure $t\bar{t}/$single top CR.


% \subsection{0-/1-lepton Monte Carlo Modelling \& Uncertainties}
% \label{sec:ttbar_MC}

% The default MC configuration for the $t\bar{t}$ process is \POWHEG + \PYTHIA8,
% which utilises the \POWHEG NLO matrix element (ME) generator
% \cite{JHEP0709.2007.126,JHEP0411.2004.040} interfaced to
% \PYTHIA8.210\cite{Comp.Phys.Comm.191.159} using the A14 tune
% \cite{ATL-PHYS-PUB-2014-021} for the parton shower, hadronization, underlying
% event, and multiple parton interactions. In addition to this, the sample
% utilises the NNPDF3.0 (NLO), and NNPDF2.3 (LO) PDF set in the ME calculation and
% parton shower respectively.


% Full details concerning the tuning, and optimisation of top-quark pair
% production ($t\bar{t}$) modelling, in addition to the systematic model used to
% cover theoretical uncertaities, can be found in
% Ref.\cite{ATL-PHYS-PUB-2016-020}. However, a brief summary of the \POWHEG
% generator configuration is provided in the following. The intrinsic $h_{damp}$
% parameter to the \POWHEG ME generator, which serves as a resummation damping
% factor to control the ME/PS matching in \POWHEG (effectively regulates the high
% $p_T$ radiation) was found in comparisons to 8 TeV data across various variables
% ($t\bar{t}$ system $p_T$, mass, top $p_T$, jet multiplicity, jet pTs and so on),
% to disfavour valued of $h_{damp} < m_{top}$. The best description was given with
% $h_{damp} = 1.5 \cdot m_{top}$ for both 8 TeV data and 13 TeV data, and so this
% value has been chosen for the main sample generation.


% Additional parameters that were optimized are the \PYTHIA8 \texttt{pTdef} and
% \texttt{pTHard} parameters, which control the merging between \POWHEG and
% \PYTHIA8 through the use of vetoed showering. Strongly correlated with the $p_T$
% definition used for ISR and FSR, and the procedure to calculate the matching
% scale, the parameters were varied with $h_{damp} = 1.5 \cdot m_{top}$. The
% resulting optimised parameter settings were identified to be \texttt{pTdef}$=2$
% and \texttt{pTHard}$=0$ (See Section 2.2.2 of Ref\cite{ATL-PHYS-PUB-2016-020}).


% A summary of all samples used as either the nominal MC prediction or the
% alternative systematic samples (detailed in Section
% \ref{subsubsec:ttbarAlternative}) are listed in
% Tab.~\ref{tabular:mc_samples_ttbar2}. Samples are filtered at the event
% generation stage using truth level information, such that all samples with
% ``non-all-had'' are required to have at least one W boson decaying leptonically,
% and ``dilepton'' samples are required to have both W bosons decaying
% leptonically. Further details concerning filters specific to each channel, like
% the \met\ filtered \ttbar\ samples (DSIDs 345935, 407345, 407346, 407347)
% utilised in the 0-lepton channel to increase the number of simulated \ttbar\
% events, can be found in Appendix~\ref{app:metfilterttbar}.
...
%         \hline
%         410470 & non-all-had $t\bar{t}$                 & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.54300 & 709,060,000 \\   %% (FS) 
%         410472 & dilepton    $t\bar{t}$                                        & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.10500 & 554,162,000 \\   %% (FS)
%         345935 & non-all-had $t\bar{t}$, 100 < \met\ < 200~GeV                   & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.09520 & 22,000,000 \\   %% (FS)
%         407345 & non-all-had $t\bar{t}$, 200 < \met\ < 300~GeV                     & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.00806 & 41,936,300 \\   %% (FS)
%         407346 & non-all-had $t\bar{t}$, 300 < \met\ < 400~GeV                       & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.00112 & 16,491,045 \\   %% (FS)
%         407347 & non-all-had $t\bar{t}$, \met\ > 400~GeV                               & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.00031 & 6,779,400 \\   %% (FS)
%         \hline\hline
%         410480 & non-all-had $t\bar{t}$ RadHi   & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.43842 & 376,240,000  \\  %% (AF)
%         410482 & dilepton $t\bar{t}$ RadHi      & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.10500 & 312,904,000  \\  %% (AF)    
%         410470 & non-all-had $t\bar{t}$ RadLo   & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.54300 & 709,060,000 \\  %% (AF)
%         410472 & dilepton $t\bar{t}$ RadLo      & \POWHEG+\PYTHIA8     & 0.83176 & 1 & 0.10500 & 554,162,000 \\  %% (AF)
%                \hline
%         410557 & single-lep  $t\bar{t}$                 & \POWHEG+\HERWIG7   & 0.83176 & 1 & 0.43842  & 37,826,500 \\   %% (FS)
%         410558 & dilepton    $t\bar{t}$                   & \POWHEG+\HERWIG7   & 0.83176 & 1 & 0.10500  & 312,507,000 \\   %% 
%         \hline                                                                                                   
%         410464 & single-lep  $t\bar{t}$                                                                                 & aMC@NLO+\PYTHIA8.2 & 0.83176 & 1 & 0.44034 & 376,985,000  \\  %% (AF)
%         410465 & dilepton    $t\bar{t}$                                                                                   & aMC@NLO+\PYTHIA8.2 & 0.83176 & 1 & 0.10718 & 364,951,000  \\  %% (AF)
%         \hline
%         \hline
% \end{tabular}
% }
% \caption{ $t\bar{t}$ samples used in the analysis, both for the nominal
...
% \label{tabular:mc_samples_ttbar2}
% \end{center}
% \end{footnotesize}
% \end{table}
...
% \label{subsubsec:ttbarXS}
% The $t\bar{t}$ cross section for pp collisions at a centre-of-mass energy of
% $\sqrt{s} = 13$TeV is $\sigma_{t\bar{t}}= 831.76^{+40}_{-46}$pb for a top quark
% mass of 172.5 GeV. It has been calculated at next-to-next-to leading order
% (NNLO) in QCD including resummation of next-to-next-to-leading logarithmic
% (NNLL) soft gluon terms with top++2.0
% \cite{Beneke2012695,Cacciari2012612,PhysRevLett.109.132001,NNLOcorr,NNLOcorrNLO,PhysRevLett.110.252004,Czakon:2011xx}.
% The PDF and $\alpha_S$ uncertainties were calculated using the PDF4LHC
...
% PDFs uncertainties by themselves correspond to a $\pm 35.06$\,pb error on the
% total cross section, while QCD scale variations correspond to an error of
% $^{+19.77}_{-29.20}$\,pb on the total cross section.


% It should be noted that all the \ttbar\ MC samples used as either the default
% $t\bar{t}$ prediction in the analysis or as a systematic variation sample, are
% normalised to the NNLO+NNLL cross-section given in Table
% \ref{tabular:mc_samples_ttbar2}.
...
%         \hline
%         \POWHEG+\PYTHIA8  &  A14 tune & nominal sample \\
%                           &  NNPDF30NLO \& NNPDF23LO & \\
%                           & \texttt{hdamp} = $1.5\cdot m_{top}$ & \\
%                           &  nonallhad filter & \\
%         \hline
%         \POWHEG+\PYTHIA8  & nominal setup & \textit{low variation} for additional radiation \\
%                           & scale variations low ($\mu_R = \mu_F = 2$) & \\
%                           & \texttt{hdamp} = $1.5\cdot m_{top}$ & \\
%                           & Up variation of A14 tune (Var3c) & \\
%                           &  nonallhad filter & \\
%         \hline
%         \POWHEG+\PYTHIA8  & nominal setup & \textit{high variation} for additional radiation \\
%                           & scale variations high ($\mu_R = \mu_F = 0.5$) & \\
%                           & \texttt{hdamp} = $3.0\cdot \times m_{top}$ & \\
%                           & Down variation of A14 tune (Var3c) & \\  
%                           &  nonallhad filter & \\
%         \hline
%         \POWHEG+\HERWIG7  & H7UE tune & fragmentation/hadronisation model\\
%                           & CT10 \& MMHT2014lo68cl & \\
%                           & \texttt{hdamp}=175.2GeV & \\
%                           & nonallhad filter & \\
%         \hline
%         \MADGRAPH5\_aMC@NLO+\PYTHIA8 & A14 tune & hard scatter generation and matching\\
%                           & NNPDF30NLO \& NNPDF23LO & \\
%                           & nonallhad filter & \\
%         \hline
%         \hline
% \end{tabular}
% }
% \caption{$t\bar{t}$ MC samples generated for the estimate of systematic uncertainties.}
% \label{tabular:ttbarAlternativeSamples}
% \end{center}
% \end{footnotesize}
% \end{table}
