...
the histograms entering in the control regions are of $P_{\mathrm{T}}^V$ and so it may be
...
nominal sample. The uncertainty relating to $S_{\mathrm{T}}$ is only applied to events
...
\texttt{EL\_EFF\_{\mathrm{T}}rigger\_{\mathrm{T}}otal\_1NPCOR\_PLUS\_UNCOR} is used to control the overall
uncertainty on the electron trigger. For the muons, the tool returns two components
\texttt{MUON\_EFF\_{\mathrm{T}}rigSystUncertainty} and \texttt{MUON\_EFF\_{\mathrm{T}}rigStatUncertainty}
...
efficient across the full $p_{\mathrm{T}}$~spectrum. Identification has an efficiency scale
factor is available from $p_{\mathrm{T}}>7$~\GeV. An isolation efficiency scale factor is
also included. The latest uncertainties that are available include scale factors
for $p_{\mathrm{T}}>150$~\GeV\ that are unity due to the available sample being to small in
...
\texttt{EL\_EFF\_ID\_{\mathrm{T}}otal\_1NPCOR\_PLUS\_UNCOR},
\texttt{EL\_EFF\_Reco\_{\mathrm{T}}otal\_1NPCOR\_PLUS\_UNCOR}, and
\texttt{EL\_EFF\_Iso\_{\mathrm{T}}otal\_1NPCOR\_PLUS\_UNCOR} in the profile-likelihood fit.
...
they come from the high or low $p_{\mathrm{T}}$ measurement. Statistical and systematic
...
They are supported in the range of $10 < p_{\mathrm{T}} < 500$~\GeV. For muons outside of
this range, a scale factor of 1$\pm$0.05 is used.
\texttt{MUON\_EFF\_{\mathrm{T}}TVA\_STAT} and \texttt{MUON\_EFF\_{\mathrm{T}}TVA\_SYS} control the
...
$p_{\mathrm{T}}$~>~20~\GeV\ and with a sample of $J/\psi\rightarrow \mu\mu$ events in region
with $p_{\mathrm{T}}$~<~20~\GeV. The nuisance parameters control the uncertainties due to
...
\texttt{TAUS\_{\mathrm{T}}RUEHADTAU\_SME\_{\mathrm{T}}ES\_DETECTOR},
\texttt{TAUS\_{\mathrm{T}}RUEHADTAU\_SME\_{\mathrm{T}}ES\_INSITU} and
\texttt{TAUS\_{\mathrm{T}}RUEHADTAU\_SME\_{\mathrm{T}}ES\_MODEL}, which all account for different
...
accounts for effects due to eta calibration, high-$p_{\mathrm{T}}$ jets, pile-up, flavour
...
Once again $E_{\mathrm{T}}^{\text{miss}}$ systematic uncertainties are implemented via a
...
$E_{\mathrm{T}}^{\text{miss}}$ in table~\ref{tab:expSyst} and start with \texttt{MET}.
...
The analysis gains a lot of sensitivity from high $p_{\mathrm{T}}^V$ regions of phase
...
slices of the larger of the $p_{\mathrm{T}}$ of the vector boson or the $H_{\mathrm{T}}$ of the event.
Events are simulated in the following slices
\begin{equation}
  \text{max}(p_{\mathrm{T}}^V, H_{\mathrm{T}}) = [0\text{--}70, 70\text{--}140, 140\text{--}280, 280\text{--}500, 500\text{--}1000, >1000]~\GeV.
\end{equation}
The dedicated slices at high $\text{max}(p_{\mathrm{T}}^V, H_{\mathrm{T}})$ ensure a large number of
events are simulated in the high $p_{\mathrm{T}}^V$ region of phase space. Secondly filters
...
The filters are not applied to the highest slices in $\text{max}(p_{\mathrm{T}}^V, H_{\mathrm{T}})$.
...
corresponding max($H_{\mathrm{T}}$,$p_{\mathrm{T}}^V$) slices and flavour filters are listed in
