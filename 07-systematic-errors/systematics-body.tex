...
ensure that probability is conserved, i.e. $y_0 + y_1 = 1$. This technique has
been demonstrated in the literature~\cite{cranmer2016approximating} where the
mathematical details are discussed in more detail.
...
shape between two predictions is the same as in the one dimensional case.
...
% function), $s(\vec{x})$, for a space of $p$-real values,
% $\mathbb{R}^{p}$, then in making the substitution $\vec{x} ->
% u=s(\bold{x})$  into the density ratio:

% \begin{equation}
%   r(\vec{x}) \rightarrow r(u) = \frac{p(u|\vec{\theta}_{1})}{p(u|\vec{\theta}_{0})},
%   \label{eq:DensityRatioPDim}
% \end{equation}
% one can prove the equivalence of equation \ref{eq:DensityRatio} and equation
% \ref{eq:DensityRatioPDim} provided $s(\vec{x})$ is a monotonic function
...
% \ref{eq:DensityRatioPDim}, is that by substituting $\vec{x} \rightarrow u
% = s(\vec{x})$ the probability density ratio in $\mathbb{R}^{p}$ space is
...
% $s(\vec{x})$ instead of an observable $\mathcal{O}_{i}$.
...
% value $s(\vec{x})$ for each event. The distribution of this is then shown
% in (c), where the ratio of two classifier responses corresponds to the
% probability density ratio $r(u=s(\vec{x}))$.
...
%     models, $s(\vec{x})$ %$s(\vec{x}; \vec{\theta}_{0},
%     % \vec{\theta}_{1})$
...
%     $r(u=s(\vec{x}))$ as a function of $s(\vec{x})$
% \end{enumerate}
...
%     classify ($s(\vec{x})$) the two MCs, whilst c) shows the
%     $s(\vec{x})$ classifier distribution and its corresponding density
%     ratio $r(\vec{x})$. }
