...
in Section~\ref{sec:ana-regions}. Each systematic uncertainty is controlled by a
...
considering all heavy flavour sub-processes of the $V+$jets background one could
...
This section describes experimental systematic uncertainties, which arise due to
...
2.4\%, and 2.0\% for the years 2015, 2016, 2017, 2018
respectively~\cite{ATLAS-CONF-2019-021}. A combined uncertainty is calculated
for the entire period 2015--2018 at 1.7\%. This figure is smaller than the
weighted sum of the relative errors on each year due to some of the sources of
uncertainty being correlated (fully or partially) between years. It is calculated
using standard error propagation taking into account the covariance matrix of
the absolute luminosity uncertainties for the different years. The full
breakdown of the uncertainties, including which sources are considered
correlated either partially or fully is given in~\cite{ATLAS-CONF-2019-021}.
...
in Section~\ref{sec:0lep-selection}) on the trigger efficiency. The uncertainty
...
are available in the ATLAS internal pile-up reweighting
tool~\cite{EgammaCalibTWiki}. Previous iterations of this analysis have proven
not to be very sensitive to these quantities~\cite{vh-obs, Hbb2017:ATLAS} and
therefore only two uncertainties are considered which are called
\texttt{EG\_RESOLUTION\_ALL} and \texttt{EG\_SCALE\_ALL}.
...
relevant nuisance parameters start with \texttt{JET}.
...
selection criteria at generator level, $m_{\ell\ell}>40$ \GeV, must be taken
into account. The NNLO calculation uses a cut of $66<m_{\ell\ell}<116$ \GeV,
which can be applied before reconstruction to the samples of this analysis in
order to get the scaling correct as follows
\begin{equation}
  k_{\text{NNLO}}^{\text{QCD}} =
...
NNLO theoretical cross section is available. Values from the
...
The nominal and alternative predictions described in Section~\ref{sec:vjets} are used to
...
equation~\ref{eq:acceptance-dr} for each sub-component, and the calculated priors
...
in Section~\ref{sec:hybrid-reweight} is used to generate uncertainties on the shape of
...
nuisance parameters are used for the 2--jet and 3--jet regions, called
...
channels. It is called \texttt{SysZbbNorm\_0L} and is applied to $Z+$hf events
...
analysis. The uncertainty is used in the 0--lepton channel which adds an
additional complication. As mentioned in section~\ref{sec:2lep-selection} a
kinematic fit is used in order to correct the $b$-jet momentum and these
corrected jets are then used in the formulation of $m_{bb}$ in the 2--lepton
channel. This means that $m_{bb}$ in the 0-- and 2--lepton channels refers to a
...
leptonic channels. The aforementioned veto on events in the range $80 \GeV <
m_{bb} < 140 \GeV$ is therefore actually applied based on GSC $m_{bb}$.
...
The subtracted data is the data minus a template comprised of the data from the
top $e\mu$ CR and the nominal predictions of all backgrounds other than
$Z+$jets. Plots are shown in the 2--jet, $\geq$3--jet and $\geq$2--jet
categories (the sum of the two jet multiplicity based categories in the
analysis) and are plotted across all $p_{\mathrm{T}}^V$ bins of the analysis.
The shape uncertainty is derived by taking a fit (with linear or logarithmic
functional form as written on each plot) to the ratio in the bottom pane of each
plot. The fit defines the 1-$\sigma$ up variation of the Gaussian
constrained nuisance parameter and the symmetrised version of the fit (not
displayed) defines the 1-$\sigma$ down variation. The $\geq$2--jet region is used
to calculate the prior for the systematic uncertainties \texttt{SysZPtV} and
\texttt{SysZMbb} that are applied to all $Z+$jets events in the 0-- and
2--lepton channels.
...
from the $75~\GeV < p_{\mathrm{T}}^V < 150~\GeV$ analysis bin, therefore the
aforementioned nuisance parameters are decorrelated in this bin to stop the
effect of opposite sign deviations between data and the prediction cancelling.
...
ratio is logarithmic, whereas the GSC $m_{bb}$ ratio is linear and its value at
...
\begin{sloppypar}
  ...
  $bc$ sub-components are non-negligible, the remainder of sub-components are
  combined into $\text{Oth}~=~[bl, cc, cl, ll]$, where $\text{Oth}$ is short for
  other. Nuisance parameters are therefore implemented for $bc$ and Oth only
  with $bb$ on the bottom of the ratio as before. Given that neither the matrix
  element nor parton shower variation dominates the uncertainty heavily,
  nuisance parameters are implemented for each in turn. In total the nuisance
  parameters controlling flavour composition uncertainty are
  \texttt{SysTTbarbcMeACC}, \texttt{SysTTbarbcPSACC}, \texttt{SysTTbarOthMeACC},
  \texttt{SysTTbarOthPSACC}.
\end{sloppypar}
...
correlated in each leptonic channel but decorrelated in 2-- and 3--jet categories as
...
in jet multiplicity but decorrelated in each leptonic channel and also in the
1--lepton channel decorrelated in each $p_{\mathrm{T}}^V$ bin, the nuisance parameters in
...
It also means we do not have to consider theoretical modelling uncertainties
...
comparison can be found in figure~\ref{fig:ttbardd-mbb}, which is split across
two figures.
