...
% %-------------------------------------------------------------------------------
% \begin{description}
%   \item[$\bullet$ luminosity] 
%     The uncertainty on the integrated luminosity is 2.1\%, 2.6\%, 2.4\%, and
%     2.0\% for the data collection years of 2015, 2016, 2017, 2018
%     respectively. For the combined integrated luminosity 2015-2018 luminosity
%     is 1.7\%. It is derived following a methodology similar to that detailed
%     in ~\cite{lumiDetermine}, from calibrations of the luminosity scale using
%     x--y beam-separation scans ~\cite{lumiTwiki}.

%   \item[$\bullet$ pile-up] compute the systematics by changing the nominal
%   data scale of $1.0/1.03$ by $1.0/1.00$ or $1.0/1.18$ to get respectively the
%   up and down variations~\cite{puTWiki}.
...
% %-------------------------------------------------------------------------------
% \subsubsection{\MET trigger}
% \label{sec:sysMETtrig}
% %-------------------------------------------------------------------------------
% \MET trigger SFs are derived using $W(\mu,\nu)$+jets events as outlined in
% Section \ref{sec:trig0lepSF}. To account for biases three effects are taken
% into account as systematic uncertainties. First the statistical error on the
% dataset from which the scale factor was derived, referred to as
% \texttt{METTrigStat}, is used as a systematic uncertainty. This is extracted
% by taking the upper and lower $1\sigma$ uncertainty band of the polynomial fit
% to the MC/Data ratio, as shown by Figure \ref{fig:trig_SFw}, as alternative
% up/down SF variations.


% The second systematic accounts for differences in SF measurement when using
% different physics processes, for example \ttbar, to derive the SFs. As such
% the trigger SFs are also derived using \ttbar events, such that the difference
% between the SF(\ttbar) with respect to SF($W(\mu,\nu)$+jets) for a given \MET
% value is defined as a systematic; \texttt{METTrigTop}.


% The third systematic, referred to as \texttt{METTrigSumPt}, accounts for the
% kinematic dependence of the \MET trigger efficiency on the offline scalar sum
% of all final state jets within the event, known as \texttt{SumPt}. Relevant
% only for the 2017 specific trigger, as shown by the bottom right plot in
% Figure \ref{fig:SF_sumpt}, the scale factors as a function of \texttt{SumPt}
% are used as an additional systematic.


% Detailed studies and results pertaining to the aforementioned systematics can
% be found in Section~\ref{sec:trig0lep}.

% %-------------------------------------------------------------------------------
% \subsubsection{Lepton trigger}
% \label{sec:sysLEPtrig}
% %-------------------------------------------------------------------------------
% The lepton trigger systematics are provided by CP groups and links to
% recommendation can be found in Section~\ref{sec:trig12lep}.


% The trigger systematics for electron are returned by CP tool as
% \texttt{EL\_EFF\_Trigger\_TotalCorrUncertainty}. It contains the systematics
% and statistical error on the trigger SF. Systematics is obtained by doing
% variation of $+/-$1 $\sigma$ of this error.

% For the muons, the CP tool returns two components : the systematics error
% (\texttt{MUON\_EFF\_TrigSystUncertainty}) and the statistical error
% (\texttt{MUON\_EFF\_TrigStatUncertainty}) on the trigger SF. Systematics are
% obtained by doing variation of $+/-$1 $\sigma$ of these errors.
