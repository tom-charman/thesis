\begin{table}
  ...
  \resizebox{\linewidth}{!}{
    \begin{tabular}{lS[table-format=3.2]S[table-format=3.2]S[table-format=3.2]S[table-format=3.2]S[table-format=3.2]S[table-format=3.2]}
      \toprule   
      {\bfseries Region ($\bm{p^{V}_{T}}$ range)} & {\bfseries CRLow (\texttt{SysZPtv} \%)} & {\bfseries SR (\texttt{SysZPtv} \%)} & {\bfseries CRHigh (\texttt{SysZPtv} \%)} & {\bfseries  Weighted Sum $\bm{p^{V}_{T}}$ (\%)} & {\bfseries $\bm{\Delta}$(SR - CRLow)} & {\bfseries $\bm{\Delta}$(CRHigh, SR)} \\
      \midrule
  ...
  uncertainty.]{SysZPtv shape effect}
    ...
    % analysis.
  \label{tab:zjets-ptv-extrap-2lep}
\end{table}