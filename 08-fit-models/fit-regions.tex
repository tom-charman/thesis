\begin{table}[h!tbp]
  \setlength{\extrarowheight}{2pt} 
  \centering
  \resizebox{\textwidth}{!}{
    \begin{tabular}{| l | l | C{1.7cm} | C{1.7cm} | C{1.9cm} | C{1.9cm} | C{1.3cm} | C{1.3cm} |}
      \toprule
      \multirow{3}{*}{\bfseries Channel}   & \multirow{3}{*}{\bfseries Region}  & \multicolumn{6}{c|}{\bfseries Categories}   \\
      \cline{3-8}
                                 &                         & \multicolumn{2}{c|}{\bfseries $\bm{75}$~\GeV$\bm{ < p_{\text{T}}^V < 150}$~\GeV} & \multicolumn{2}{c|}{\bfseries $\bm{150}$~\GeV$\bm{ < p_{\text{T}}^V < 250}$~\GeV} & \multicolumn{2}{c|}{\bfseries $\bm{p_{\text{T}}^V > 250}$~\GeV} \\
      \cline{3-8}
                                 &                         & {\bfseries $\bm{2}$--jets} & {\bfseries $\bm{3}$--jets} & {\bfseries $\bm{2}$--jets} & {\bfseries $\bm{3}$--jets} & {\bfseries $\bm{2}$--jets} & {\bfseries $\bm{3}$--jets} \\
      \midrule
      $0$--lepton                 & CRLow        & $-$      & $-$      & yields    & yields    & yields    & yields      \\
                                 & SR           & $-$      & $-$      & BDT       & BDT       & BDT       & BDT      \\
                                 & CRHigh       & $-$      & $-$      & yields    & yields    & yields    & yields      \\
      \midrule
      $1$--lepton                 & CRLow        & $-$      & $-$      & yields    & yields    & yields    & yields      \\
                                 & SR           & $-$      & $-$      & BDT       & BDT       & BDT       & BDT      \\
                                 & CRHigh       & $-$      & $-$      & yields    & yields    & yields    & yields      \\
      \midrule
      $2$--lepton                 & CRLow        & yields   & yields   & yields    & yields    & yields    & yields      \\
                                 & SR           & BDT      & BDT      & BDT       & BDT       & BDT       & BDT      \\
                                 & CRHigh       & yields   & yields   & yields    & yields    & yields    & yields      \\
      \bottomrule
    \end{tabular}
  }
\caption{Event categories and discriminants fitted to data in the 0--, 1-- and
  $2-$lepton channels.}
\label{tab:fit-regions}
\end{table}