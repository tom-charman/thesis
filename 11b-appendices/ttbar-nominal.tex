\begin{table}[!htb]
  \centering
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{rllS[table-format=3.2]lS[table-format=3.2]r}
      \toprule
      DS ID & Process & Generator &  $\sigma\times\text{BR}$ [nb] & $k$-factor & $\epsilon_{\text{filter}}$ & Events  \\ 
      \midrule
      410470 & non-all-had $t\bar{t}$ & \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.54300 & 709,060,000 \\   %% (FS) 
      410472 & dilepton    $t\bar{t}$ & \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.10500 & 554,162,000 \\   %% (FS)
      345935 & non-all-had $t\bar{t}$, 100 < $E_T^{\text{miss}}$\ < 200~GeV & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.09520 & 22,000,000 \\
      407345 & non-all-had $t\bar{t}$, 200 < $E_T^{\text{miss}}$\ < 300~GeV & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.00806 & 41,936,300 \\
      407346 & non-all-had $t\bar{t}$, 300 < $E_T^{\text{miss}}$\ < 400~GeV & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.00112 & 16,491,045 \\
      407347 & non-all-had $t\bar{t}$, $E_T^{\text{miss}}$\ > 400~GeV  & \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.00031 & 6,779,400 \\
      410480 & non-all-had $t\bar{t}$ RadHi   & \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.43842 & 376,240,000  \\  %% (AF)
      410482 & dilepton $t\bar{t}$ RadHi      & \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.10500 & 312,904,000  \\  %% (AF)    
      410470 & non-all-had $t\bar{t}$ RadLo   & \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.54300 & 709,060,000 \\  %% (AF)
      410472 & dilepton $t\bar{t}$ RadLo      & \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.10500 & 554,162,000 \\  %% (AF)
      410557 & single-lep  $t\bar{t}$         & \textsc{Powheg}~+~\textsc{Herwig}~7   & 0.83176 & 1 & 0.43842  & 37,826,500 \\   %% (FS)
      410558 & dilepton    $t\bar{t}$         & \textsc{Powheg}~+~\textsc{Herwig}~7   & 0.83176 & 1 & 0.10500  & 312,507,000 \\   %% 
      410464 & single-lep  $t\bar{t}$         & aMC@NLO~+~\textsc{Pythia}~8.2 & 0.83176 & 1 & 0.44034 & 376,985,000  \\  %% (AF)
      410465 & dilepton    $t\bar{t}$         & aMC@NLO~+~\textsc{Pythia}~8.2 & 0.83176 & 1 & 0.10718 & 364,951,000  \\  %% (AF)
      \bottomrule
    \end{tabular}
  }
  \caption{Nominal ttbar samples.}
  % $t\bar{t}$ samples used in the analysis, both for the nominal
  % prediction and the systematic variations. The dataset ID, MC generator,
  % production cross-section, $k$-factor, filter efficiency and total number of
  % generated events are shown. Note that all samples are normalised to the same
  % NNLO cross-section value. The $E_T^{\text{miss}}$\ filtered \ttbar\ samples
  % are utilised in % the 0-lepton channel to increase the number of simulated
  % \ttbar\ events, with % more details provided in
  % Appendix~\ref{app:metfilterttbar}.
  \label{tab:ttbar-nominal}
\end{table}

\begin{table}[!htb]
  \centering
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{clccccr}
      \toprule
      DS ID & Process & Generator &  $\sigma\times\text{BR}$ [nb] & $k$-factor & $\epsilon_{\text{filter}}$ & Events  \\ 
      \midrule
      410470 & non-all-had $t\bar{t}$ & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.54386 & 286,547,351 \\   %% (FS)
      410472 & dilepton    $t\bar{t}$ & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.10537 & 216,642,783 \\  %% (FS)
      345935 & non-all-had $t\bar{t}$, 100 < $E_T^{\text{miss}$ < 200~GeV  & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.0952 & 70,312,600 \\
      407345 & non-all-had $t\bar{t}$, 200 < $E_T^{\text{miss}$ < 300~GeV & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.0080 & 41,009,929 \\
      407346 & non-all-had $t\bar{t}$, 300 < $E_T^{\text{miss}$ < 400~GeV & \textsc{Powheg}~+~\textsc{Pythia}~8 & 0.83176 & 1 & 0.0011 & 15,967,249 \\
      407347 & non-all-had $t\bar{t}$, $E_T^{\text{miss}$ > 400~GeV 		& \textsc{Powheg}~+~\textsc{Pythia}~8     & 0.83176 & 1 & 0.0003 & 6,725,364 \\
      \bottomrule
    \end{tabular}
    \caption{ $t\bar{t}$ $E_T^{\text{miss}$ filtered samples generated for use in the
      0--lepton channel.}
    \label{tab:ttbar-metfilter}
\end{table}